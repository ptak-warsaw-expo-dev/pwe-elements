 ...
 */
class PWE_Conference_Cap_Legacy_Renderer {
    ...
    public function addingStyles(){
        $css_file = PWE_CONFERENCE_CAP_URL . 'assets/common/conference_cap.css';
        $css_version = filemtime(PWE_CONFERENCE_CAP_PATH . 'assets/common/conference_cap.css');
    ...
    public static function addingScripts($atts , $speakersDataMapping, $one_conf_mode = false, $archive = '') {
        PWE_Conference_Cap_Assets::enqueue_runtime($speakersDataMapping, (bool) $one_conf_mode, (string) $archive);
    }

    public static function PWEConferenceCapOutput($atts) {

        require_once PWE_CONFERENCE_CAP_PATH . 'classes/conference-cap-functions.php';
        $conf_function = new PWEConferenceCapFunctions;

        $attributes = PWE_Conference_Cap_Attributes::from_shortcode(is_array($atts) ? $atts : array())->to_array();
        $conference_cap_html = $attributes['conference_cap_html'];
        $conference_cap_conference_mode = $attributes['conference_cap_conference_mode'];
        $conference_cap_conference_arichive = $attributes['conference_cap_conference_arichive'];
        $atts = $attributes;
        ...
        if (strpos($_SERVER['HTTP_HOST'], 'warsawexpo.eu') !== false) {
            require_once PWE_CONFERENCE_CAP_PATH . 'modes/warsawexpo/warsawexpo.php';
        ...
        if(!empty($isWeek) && $_SERVER['HTTP_HOST'] == 'warsawsecuritydefenceexpo.com'){
        ...
        if (!empty($conference_cap_html)) {
            foreach ($conference_cap_html as $conf_cap_html) {
                // Get the target conference slug
        ...
        // Load and prepare manual conferences
        $manual_conferences = [];
        ...
        $output .= '<div id="conference-cap" class="conference_cap__main-container">';
            ...
            if (!empty($database_data)) {
                ...
                // Process 'conference_cap_conference_display_slug'
                ...
                foreach ($database_data as $conf) {
                    $slug  = $conf->conf_slug ?? '';
                    $links = strtolower($conf->conf_site_link ?? '');
                    $links = preg_replace('/\s+/', ' ', $links);

                    $pattern = '/(?:^|[,;\s])(?:www\.)?' . preg_quote($domain, '/') . '\s*\[(\d+)\]/';

                    if ($slug && preg_match($pattern, $links, $m)) {
                        $orderMap[$slug] = (int) $m[1];
                    }
                }

                uksort($conf_slugs, function($a_slug, $b_slug) use ($orderMap) {
                    $orderA = $orderMap[$a_slug] ?? 999;
                    $orderB = $orderMap[$b_slug] ?? 999;
            ...
            $output .= PWEConferenceCapFunctions::debugConferencesConsole( $database_data );

            if (!$one_conf_mode) {
                $output .= '<div class="conference_cap__conf-slug-navigation">';

                        // 2. Manual conferences from VC data
                        $has_valid_manual_conf = !empty(array_filter($manual_conferences, function($conf) {
                            return !empty($conf['manual_conf_img']) && !empty($conf['manual_conf_id']);
                        }));

                        // Prepare manual entries and slugs to hide at this point
                        $new_manual_conferences = [];
                        $manual_slugs_to_hide = [];
                        ...
                        foreach ($conf_slugs as $conf_slug => $conferences) {
                            ...
                            if (strpos($conf_slug, 'ceremonia-medalowa') !== false) {
                        ...
                        // 1. Normal conferences from the database
            ...
            $output .= '<div class="conference_cap__conf-slugs-container">';
                ...
                foreach ($conf_slugs as $conf_slug => $conferences) {
                    ...
                    require_once PWE_CONFERENCE_CAP_PATH . $conference_modes['php'];
                    if (!class_exists($new_class)) {
                        $new_class = 'PWEConferenceCapFullMode';
                    }
                    $mode_class = new $new_class;

                    $css_handle = 'conference-style-' . sanitize_title($conf_style);

                    $css_file = PWE_CONFERENCE_CAP_URL . $conference_modes['css'];
                    if (file_exists(PWE_CONFERENCE_CAP_PATH . $conference_modes['css'])) {
                        $css_version = filemtime(PWE_CONFERENCE_CAP_PATH . $conference_modes['css']);
                        wp_enqueue_style($css_handle, $css_file, array(), $css_version);
                    }

                    // Container for one conference
                    $output .= '<div id="conf_' . esc_attr($conf_slug) . '" class="conference_cap__conf-slug">';
                        $output .= '<div class="conference_cap__conf-slug-header">';
                            if (!empty($conf_img) && (get_class($mode_class) !== 'PWEConferenceCapMedalCeremony')) {
                                ...
                                if (!empty($conf->conf_patrons_img)) {
                                    ...
                                    if (!empty($logo_files)) {
                                        include_once PWE_CONFERENCE_CAP_PATH . '/../../scripts/slider.php';
                                ...
                                // Add the main conference picture
                        ...
                        // End temporary compatibility rule
                        ...
                        if (get_class($mode_class) !== 'PWEConferenceCapMedalCeremony') {
                            ...
                            if($panel === true){
                                $trends_panel = new PWE_Conference_Cap_Trends_Panel();
                                $output .= $trends_panel->render(array('attributes' => $atts, 'language' => $lang));
                            }
                        }
                    $output .= '</div>';
                    if ($one_conf_mode) {
